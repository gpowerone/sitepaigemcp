...
export async function writeModelsSql(targetDir: string, blueprint: Blueprint, databaseType: "postgres" | "sqlite" | "mysql" = "postgres"): Promise<void> {
  ...
  if (databaseType === 'sqlite') {
    passwordAuthTable.push(
      `CREATE TABLE IF NOT EXISTS ${quoteChar}passwordauth${quoteChar} (`,
      `  ${quoteChar}id${quoteChar} TEXT PRIMARY KEY,`,
      `  ${quoteChar}email${quoteChar} TEXT NOT NULL UNIQUE,`,
      `  ${quoteChar}passwordhash${quoteChar} TEXT NOT NULL,`,
      `  ${quoteChar}salt${quoteChar} TEXT NOT NULL,`,
      `  ${quoteChar}verificationtoken${quoteChar} TEXT,`,
      `  ${quoteChar}verificationtokenexpires${quoteChar} TEXT,`,
      `  ${quoteChar}emailverified${quoteChar} INTEGER DEFAULT 0,`,
      `  ${quoteChar}resettoken${quoteChar} TEXT,`,
      `  ${quoteChar}resettokenexpires${quoteChar} TEXT,`,
      `  ${quoteChar}createdat${quoteChar} TEXT DEFAULT CURRENT_TIMESTAMP,`,
      `  ${quoteChar}updatedat${quoteChar} TEXT DEFAULT CURRENT_TIMESTAMP`,
      `);`
    );
  } else if (databaseType === 'postgres') {
    passwordAuthTable.push(
      `CREATE TABLE IF NOT EXISTS ${quoteChar}passwordauth${quoteChar} (`,
      `  ${quoteChar}id${quoteChar} UUID PRIMARY KEY,`,
      `  ${quoteChar}email${quoteChar} VARCHAR(255) NOT NULL UNIQUE,`,
      `  ${quoteChar}passwordhash${quoteChar} TEXT NOT NULL,`,
      `  ${quoteChar}salt${quoteChar} TEXT NOT NULL,`,
      `  ${quoteChar}verificationtoken${quoteChar} TEXT,`,
      `  ${quoteChar}verificationtokenexpires${quoteChar} TIMESTAMP,`,
      `  ${quoteChar}emailverified${quoteChar} BOOLEAN DEFAULT FALSE,`,
      `  ${quoteChar}resettoken${quoteChar} TEXT,`,
      `  ${quoteChar}resettokenexpires${quoteChar} TIMESTAMP,`,
      `  ${quoteChar}createdat${quoteChar} TIMESTAMP DEFAULT CURRENT_TIMESTAMP,`,
      `  ${quoteChar}updatedat${quoteChar} TIMESTAMP DEFAULT CURRENT_TIMESTAMP`,
      `);`
    );
  } else if (databaseType === 'mysql') {
    passwordAuthTable.push(
      ...
      `  ${quoteChar}salt${quoteChar} TEXT NOT NULL,`,
      `  ${quoteChar}verificationtoken${quoteChar} TEXT,`,
      `  ${quoteChar}verificationtokenexpires${quoteChar} DATETIME,`,
      `  ${quoteChar}emailverified${quoteChar} BOOLEAN DEFAULT FALSE,`,
      `  ${quoteChar}resettoken${quoteChar} TEXT,`,
      `  ${quoteChar}resettokenexpires${quoteChar} DATETIME,`,
      `  ${quoteChar}createdat${quoteChar} DATETIME DEFAULT CURRENT_TIMESTAMP,`,
      `  ${quoteChar}updatedat${quoteChar} DATETIME DEFAULT CURRENT_TIMESTAMP`,
      `);`
    );
  }
  ...
  if (databaseType === 'sqlite') {
    lines.push(
      `CREATE TABLE IF NOT EXISTS ${quoteChar}form_submissions${quoteChar} (`,
      `  ${quoteChar}id${quoteChar} INTEGER PRIMARY KEY AUTOINCREMENT,`,
      `  ${quoteChar}timestamp${quoteChar} TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,`,
      `  ${quoteChar}form_name${quoteChar} TEXT NOT NULL,`,
      `  ${quoteChar}form_data${quoteChar} TEXT NOT NULL`,
      `);`
    );
  } else if (databaseType === 'postgres') {
    lines.push(
      `CREATE TABLE IF NOT EXISTS ${quoteChar}form_submissions${quoteChar} (`,
      `  ${quoteChar}id${quoteChar} SERIAL PRIMARY KEY,`,
      `  ${quoteChar}timestamp${quoteChar} TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,`,
      `  ${quoteChar}form_name${quoteChar} VARCHAR(255) NOT NULL,`,
      `  ${quoteChar}form_data${quoteChar} JSONB NOT NULL`,
      `);`
    );
  } else if (databaseType === 'mysql') {
    lines.push(
      ...
      `  ${quoteChar}timestamp${quoteChar} TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,`,
      `  ${quoteChar}form_name${quoteChar} VARCHAR(255) NOT NULL,`,
      `  ${quoteChar}form_data${quoteChar} JSON NOT NULL`,
      `);`
    );
  }
