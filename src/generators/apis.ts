...
import { Blueprint, Api, Code, ObjectDefinition, ObjectProperty } from "../types.js";
...
function generateTypeFromProperty(prop: ObjectProperty, objects: ObjectDefinition[]): string {
  switch (prop.type) {
    case 'string':
      return 'string';
    case 'number':
...
function generateInterface(obj: ObjectDefinition, objects: ObjectDefinition[]): string {
...
function generateDummyValue(prop: ObjectProperty, objects: ObjectDefinition[]): any {
  switch (prop.type) {
    case 'string':
...
function generateDummyObject(obj: ObjectDefinition, objects: ObjectDefinition[]): string {
...
function generateApiStub(api: Api, blueprint: Blueprint, handler: string): string {
...
export async function writeApis(targetDir: string, blueprint: Blueprint, projectCode: Code): Promise<void> {
...
`;
    ...
    const relativePathToRoot = '../'.repeat(depth); // +2 for api/routename
