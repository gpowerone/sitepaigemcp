...
async function runGenerateJob(
  ...
  try {
    ...
    if (debug) jobs.appendLog(jobId, `Debug: invoking Sitepaige API`);
...
async function runWriteJob(
  ...
  try {
    ...
    const debugEnv = process.env.SITEPAIGE_DEBUG;
    const debug = typeof debugEnv === "string" && ["1", "true", "yes", "on"].includes(debugEnv.toLowerCase());
    if (debug) jobs.appendLog(jobId, `Debug: fetching project from /api/project and writing to disk`);
    let project, wroteTo;
    try {
...
server.tool(
  ...
  async ({ 
    ...
    const colorScheme: FullColorScheme = {
      website: {
...
server.tool(
  ...
  async ({ jobId, projectId, targetDir, buildId }) => {
    ...
    let actualTargetDir: string | undefined;
    let actualDatabaseType: "sqlite" | "postgres" | "mysql" | undefined;
    let job: any = null;
            ...
            try {
              ...
              if (dirExists) {
                ...
                const significantEntries = entries.filter(e => 
                  !e.startsWith('.') && e !== 'node_modules' && e !== '__pycache__'
                );
...
server.resource("job_statuses", "mem://jobs/{jobId}/status", { mimeType: "application/json" }, async (uri) => {
  ...
  if (!res) throw new McpError(ErrorCode.InvalidParams, `Unknown resource: ${uri}`);
  let text = res.body;
  try {
    const status = JSON.parse(res.body);
    if (status && typeof status === "object" && typeof status.status === "string") {
      const map: Record<string, string> = { running: "generating", completed: "complete" };
      const external = { 
        ...status, 
  ...
  return { contents: [{ uri: uri.toString(), mimeType: res.mimeType, text }] } as any;
});

server.resource("job_logs", "mem://jobs/{jobId}/logs", { mimeType: "text/plain" }, async (uri) => {
  const res = jobs.readResource(uri.toString());
  if (!res) throw new McpError(ErrorCode.InvalidParams, `Unknown resource: ${uri}`);
  return { contents: [{ uri: uri.toString(), mimeType: res.mimeType, text: res.body }] } as any;
});
