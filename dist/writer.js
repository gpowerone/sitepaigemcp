...
export function applyBundleToDisk(options) {
    ...
    const result = { created: [], updated: [], skipped: [], conflicts: [], backups: [] };
    const fileMap = new Map();
    for (const f of files)
        fileMap.set(normalizeRelPath(f.path), f);
    ...
    for (const entry of manifest) {
        ...
        switch (overwriteMode) {
            ...
            default: {
                const _exhaustive = overwriteMode;
...
function uniqueBackupPath(originalAbsPath) {
