...
function collectImageUuidsFromBlueprint(blueprint) {
    ...
    for (const row of sample) {
        const sql = row.sql || "";
        for (const m of sql.matchAll(pat1))
            add(m[1]);
        for (const m of sql.matchAll(pat2))
            add(m[1]);
    }
    return imageInfo;
}
async function detectExtensionFromBytes(buf) {
    if (buf.length >= 12) {
...
export async function downloadImagesToPublic(targetDir, imageInfoMap) {
    const map = new Map();
    if (!imageInfoMap.size)
        return map;
    ...
    const jobs = Array.from(imageInfoMap.values()).map(async (info) => {
        ...
        try {
            ...
            if (contentType.includes("application/json") || contentType.includes("text/json")) {
                ...
                let base64OrDataUrl = undefined;
                let mimeFromPayload = undefined;
                ...
                if (!hintedExt)
                    hintedExt = extFromContentType(contentType);
                const arrayBuffer = await res.arrayBuffer();
                buf = Buffer.from(arrayBuffer);
            }
            if (!buf)
                throw new Error("No image bytes decoded for " + id);
...
export function replaceImageRefsInBlueprint(blueprint, imageMap) {
...
export async function processBlueprintImages(targetDir, blueprint) {
