...
export function generateStyleProps(systemView, isContainer) {
    const styleProps = [];
    const cssClasses = [];
    ...
    if (systemView.paddingLeft)
        styleProps.push(`paddingLeft: '${systemView.paddingLeft}px'`);
    if (systemView.paddingRight)
        styleProps.push(`paddingRight: '${systemView.paddingRight}px'`);
    if (systemView.paddingTop)
        styleProps.push(`paddingTop: '${systemView.paddingTop}px'`);
    if (systemView.paddingBottom)
        styleProps.push(`paddingBottom: '${systemView.paddingBottom}px'`);
    // Margin - using conditional checks to match preview
    if (systemView.marginLeft)
        styleProps.push(`marginLeft: '${systemView.marginLeft}px'`);
    if (systemView.marginRight)
        styleProps.push(`marginRight: '${systemView.marginRight}px'`);
    if (systemView.marginTop)
        styleProps.push(`marginTop: '${systemView.marginTop}px'`);
    if (systemView.marginBottom)
        styleProps.push(`marginBottom: '${systemView.marginBottom}px'`);
    // Min/Max dimensions - using conditional checks to match preview
    if (systemView.minHeight)
        styleProps.push(`minHeight: '${systemView.minHeight}px'`);
    if (systemView.minWidth)
        styleProps.push(`minWidth: '${systemView.minWidth}px'`);
    if (systemView.maxHeight)
        styleProps.push(`maxHeight: '${systemView.maxHeight}px'`);
    if (systemView.maxWidth)
    ...
    const placeItems = verticalAlign.toLowerCase() === 'top' && align.toLowerCase() === 'left' ? 'start start' :
...
function buildResponsiveColClasses(cfg, legacy, colsPerView) {
    const col = cfg.colpos || 1;
    const colMd = cfg.colposmd || col;
    const colSm = cfg.colpossm || colMd;
    const spanBase = (n) => Math.max(1, Math.min(12, n));
...
function parseContainerSubviews(desc) {
    if (!desc)
        return [];
                ...
                try {
                    ...
                    if (transpiledResult.success) {
                        ...
                        if (v.custom_view_description) {
                            ...
                            const fallbackResult = JSON.parse(transpiler.transpileCode(cleanedDescription, pages, blueprint.dictionary || {}));
