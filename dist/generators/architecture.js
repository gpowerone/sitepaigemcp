...
export async function writeArchitectureDoc(targetDir, blueprint) {
...
function generateArchitectureContent(blueprint) {
    const sections = [];
    // Header
            ...
            if (model.fields && model.fields.length > 0) {
                ...
                for (const field of model.fields) {
                    ...
                    if (field.is_image === 'true')
                        notes.push('Image');
                    if (field.is_file === 'true')
                        notes.push('File');
                    if (field.untouchable === 'true')
                        notes.push('System');
...
function generateObjectTable(obj, allObjects) {
    const lines = [];
...
function groupViewsByType(views) {
    const grouped = {};
    for (const view of views) {
